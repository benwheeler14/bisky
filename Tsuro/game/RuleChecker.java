...
public class RuleChecker {
	 ...
	 * @param board
	 * @param position
	 * @return whether the position on board is a valid one for the initial placement of a playertoken
	 */
	...
	public static boolean validTilePlacement(TestableBoard board, String playerName, Tile tile, Hand hand) {
		//check if the tile exists the hand( prevents player agents from making up tiles)
		if (!hand.hasMatchingTile(tile)) {
			Logging.print("Player: " + playerName + " disqualified because the given tile does not match one in the hand");
			return false;
		}
		...
		for (Tile t : hand.getAllTiles()) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (!doesMoveKillPlayer(board, playerName, t)) {
					Logging.print("Player: " + playerName + " has been disqualified because the given tile kills the player");
					return false;
				}
			}
		}
	 ...
	 * @return
	 */
	public static boolean doesMoveKillPlayer(TestableBoard board, String playerName, Tile tile) {
